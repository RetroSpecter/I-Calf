...
public class RaymarchedClouds : MonoBehaviour
{
    ...
    // Fix : Plane invisible on vesion 5.0 if enabled inside OnPreCull
    void LateUpdate()
    {
#if UNITY_5_4_OR_NEWER
    ...
    void VerifPlane(ref GameObject planeGO, ref MeshRenderer planeRenderer, ref Material material, string defaultName)
    {
        ...
        {
            /*
            foreach (Transform child in thisCam.transform.GetComponentsInChildren<Transform>().Where(child => child.name == defaultName))
